    ...
    try:
        ...
        while traffic_light_active:
            ...
            for road, count in sorted_roads:
                ...
                # Use DQN agent
                # Deterministic selection (no DQN)
                selected_road = road
                # If you want a fallback in case 'road' has no traffic, you can choose the next non-empty:
                if traffic_counts.get(selected_road, 0) <= 0:
                    # fallback: pick the first non-empty direction
                    for r in ["North", "South", "East", "West"]:
                        if traffic_counts.get(r, 0) > 0:
                            selected_road = r
                            break

                # Optionally compute next_counts and congestion if needed, but do NOT call any dqn_agent methods.
                # Example: (if you used reward_history for something else, you can keep congestion_history updates)
                next_counts = traffic_counts.copy()
                next_counts[selected_road] = max(0, next_counts[selected_road] - 1)

                # Update congestion history if you want
...
async def all_cases_feed():
    ...
    try:
        ...
        cv2.putText(placeholder, "Starting all cases simulation...", (400, 375), 
                    cv2.FONT_HERSHEY_SIMPLEX,   "Starting all cases simulation...", (400, 375), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
        _, buffer = cv2.imencode(".jpg", placeholder)
